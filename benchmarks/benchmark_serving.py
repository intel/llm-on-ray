...
async def benchmark(
    ...
    async for request in get_request(input_requests, request_rate):
        tasks.append(
            asyncio.create_task(
                send_request(
                    api_url, *request, config, track_token_latency, track_input_output, progress_bar, tokenizer
                )
            )
        )
...
def main(args: argparse.Namespace):
    ...
    if args.dataset_format == "ShareGPT":
        ...
        input_requests = sample_requests_IPEX(
            args.dataset, args.input_tokens, args.model_type, args.max_new_tokens, args.num_prompts, tokenizer
        )
    ...
    input_num_tokens = sum(
        prompt_len for _, _, prompt_len, _, _, _ in latency_tracking
    )
    output_num_tokens = sum(
        output_len for _, _, _, output_len, _, _ in latency_tracking
    )
            ...
            json.dump(
                {
                    ...
                    "output_throughput_tokens_per_s": float(f"{output_throughput_tokens_per_s:.3f}"),
