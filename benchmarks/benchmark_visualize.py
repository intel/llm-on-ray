...
def extract_metric_choice_1_2(bs_dirs, mark_name):
    bs = []
    metric_value = []
    bs_listdir = os.listdir(bs_dirs).sort(key=len)
    for bs_dir in bs_listdir:
        ...
        res_listdir = [res_i for res_i in res_listdir if "summary" in res_i].sort(key=len)
