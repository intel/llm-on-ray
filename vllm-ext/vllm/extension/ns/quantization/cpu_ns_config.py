...
def copy_quant_ns_config(model_config: ModelConfig,
    ...
    if not possible_config_filenames:
        logger.warn("ns quantization config file name not specified for model %s. Will use default ns config", model_name_or_path)
        return
    ...
    if os.path.exists(quant_config_path):
        ...
        return
    downloaded_model_name = Path(hf_folder).parent.parent.name # e.g., hub/models--meta-llama--Llama-2-7b-chat-hf/snapshots/f5db02db724555f92da89c216ac04704f23d4590/
    src_file = os.path.join(Path(__file__).parent.absolute(), downloaded_model_name, possible_config_filenames[0])
    if not os.path.exists(src_file):
        logger.warn("no ns quantization config file found in %s. Will use default ns config", src_file)
        return
...
class NSQuantConfig(QuantizationConfig):
    def __init__(self, quant_method="ns", weight_dtype="int8", group_size="128", scale_dtype="fp32", compute_dtype="int8",
                 alg="sym", version="v1"):
