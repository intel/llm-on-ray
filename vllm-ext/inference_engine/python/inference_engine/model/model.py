...
class Model:
    ...
    def __set_pad_token(self, tokenizer):
        if self.generation_args["pad_token"] == -1:
            ...
            if tokenizer.pad_token_id:
                assert tokenizer.pad_token_id == self.generation_args["pad_token"], "different pad token ids in tokenizer and model init"
            else:
    ...
    def __call__(self, input_ids_dataptr: int,
        ...
        return self.cpp_module.generate(self.native_model_ptr,
                                    ...
                                    context_lens_arr, len(context_lens)
                                )
