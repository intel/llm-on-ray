...
class PredictCallable:
    ...
    def __call__(self, batch: pd.DataFrame) -> pd.DataFrame:
        tokenized = self.tokenizer(list(batch["prompt"]), return_tensors="pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="EleutherAI/gpt-j-6B", type=str, help="model name or path"
    )
    ...
    preds = ds.repartition(100).map_batches(
        PredictCallable,
        batch_size=4,
        fn_constructor_kwargs=dict(model_id=args.model, max_new_tokens=args.max_new_tokens),
        compute="actors",
    )
