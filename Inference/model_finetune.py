...
# from run_clm_no_trainer_ray_resum_3.py
import argparse
import logging
import math
import os
...
def parse_args():
    ...
    parser.add_argument(
        "--ARROW_LIBHDFS_DIR", type=str, default=None, help="Set the path of libhdfs3."
    )
    parser.add_argument(
        "--CLASSPATH", type=str, default=None, help="Set the classpath for hdfs."
    )
    parser.add_argument(
...
def train_func(config: Dict[str, Any]):
    args = config["args"]
    new_model_save_pth = config["new_model_save_pth"]
    ...
    if args.tokenizer_name:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, use_fast=not args.use_slow_tokenizer)    # pass
    else:
        raise ValueError(
    ...
    if checkpoint_hdfs_path is not None:
        ...
        if resume_from_checkpoint:
            ...
            training_step = checkpoint_dict["step"] + 1
            print("training_step: ", training_step)
            # update the progress_bar if load from checkpoint
            progress_bar.update(starting_epoch * num_update_steps_per_epoch + training_step + 1)
            completed_steps = starting_epoch * num_update_steps_per_epoch


    for epoch in range(starting_epoch, args.num_train_epochs):
        model.train()
        for step_train, batch in enumerate(train_dataloader):
            if epoch == starting_epoch and step_train < training_step:
                continue
        ...
        if new_model_save_pth:
            unwrapped_model = accelerator.unwrap_model(model)
            checkpoint = HuggingFaceCheckpoint.from_model(unwrapped_model, tokenizer, path=new_model_save_pth)
            checkpoint.to_directory(new_model_save_pth)

def finetune_func(model_name, dataset, new_model_save_pth):
    args = parse_args()
    ...
    args.num_workers = 2
    args.address = "10.0.0.133"

    config = {"args": args, "new_model_save_pth": new_model_save_pth}
    ...
    if args.start_local or args.address or args.num_workers > 1:
        ...
        #from ray.train.huggingface import HuggingFaceCheckpoint
        #checkpoint = HuggingFaceCheckpoint.from_checkpoint(results.checkpoint)
        #hf_model = checkpoint.get_model(model=AutoModelForCausalLM)
        #hf_model.save_pretrained(new_model_save_pth)
